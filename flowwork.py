...
cwd='/Users/kevin/Desktop/photoface2/'
classes=os.listdir('/Users/kevin/Desktop/photoface2')  #54 classes
#writer= tf.python_io.TFRecordWriter("dog_and_cat_train.tfrecords") 
writer= tf.python_io.TFRecordWriter('/Users/kevin/Desktop/record/cnn.tfrecords') #generate tfrecords file
i = 0
for index,name in enumerate(classes):
    ...
    for img_name in os.listdir(class_path): 
        if img_name=='.DS_Store':
            ...
            img_path=class_path+img_name #path for each image
...
def read_and_decode(filename): # read tfrecords
    filename_queue = tf.train.string_input_producer([filename],shuffle=True)#genarate a queue
    ...
    img = tf.reshape(img, tf.stack([128, 128, 3]))  #reshape to 128*128*3
    img = tf.cast(img, tf.float32) * (1. / 255) - 0.5 #generate img tensor
    label = tf.cast(features['label'], tf.float32) #generate label tensor
...
y_ = tf.placeholder(tf.float32, [batch_size,54])
...
img, label = read_and_decode('/Users/kevin/Desktop/record/cnn.tfrecords')
...
with tf.Session() as sess:
    ...
    batch_idxs = int(1796/batch_size)
    for i in range(epoch):
        for j in range(batch_idxs):
            val, l = sess.run([img_batch, label_batch])
            l = one_hot(l,54)
            _, acc = sess.run([train_step, accuracy], feed_dict={x: val, y_: l, keep_prob: 0.6})
